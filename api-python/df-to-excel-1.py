...
#print(order)
#print(order.apply(lambda row: str(row["order_id"]) + " " + str(row["order_type"]), axis=1))
def myfunc(row):
    print(row)
    # can add custom transform, save, read here
    return row
order.apply(myfunc, axis=1)

for i, j in order.iterrows():
    print(i)
    print()

#csv_df = csv_df.astype('int')
#csv_df.loc[:, csv_df.columns != 'Date'].astype(int)
#csv_df.loc[:, csv_df.columns != 'States'].astype(int)
''' Converting all columnt to int type except Dates and States '''
cols=[i for i in csv_df.columns if i not in ["Date","States"]]
for col in cols:
